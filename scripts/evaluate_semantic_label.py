...
parser.add_argument('--gt_path', default='semantic_val', help='path to gt files')
...
def write_result_file(confusion, ious, filename):
    with open(filename, 'w') as f:
        ...
        for i in range(len(VALID_CLASS_IDS)):
            ...
            #print(label_name, ious[label_name])
...
def evaluate(pred_files, gt_files, output_file):
    ...
    for i in range(len(VALID_CLASS_IDS)):
        ...
        continue
    print('mean IOU', np.mean(list(class_ious.values()), axis=0))
    # for i in range(len(VALID_CLASS_IDS)):
    #     label_name = CLASS_LABELS[i]
