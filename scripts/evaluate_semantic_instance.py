...
parser.add_argument('--gt_path', default='instance_val', help='path to directory of gt .txt files')
parser.add_argument('--output_file', default='', help='output file [default: pred_path/semantic_instance_evaluation.txt]')
parser.add_argument('--scene_id', default='', type=str, help='scene id to test')
parser.add_argument('--num_testing_images', default=100, type=int, help='num images to test')
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                ...
                for m in matches:
                    ...
                    #print(label_name, 'num', len(gt_instances), len(pred_instances))                    
                    ...
                    for (gti,gt) in enumerate(gt_instances):
                        ...
                        #print(gti, num_pred, len(gt['matched_pred']))
                    ...
                    if len(gt_instances) > 0 and False:
                        exit(1)
                    ...
                    for pred_index, pred in enumerate(pred_instances):
                        found_gt = False
...
def print_results(avgs):
    ...
    for (li,label_name) in enumerate(CLASS_LABELS):
        ...
        line  = "{:<15}".format(label_name + ' ' + str(VALID_CLASS_IDS[li])) + sep + col1
...
def main():
    ...
    #print(pred_files)
    #['scene0217_00.txt', 'scene0488_01.txt', 'scene0412_00.txt', 'scene0412_01.txt', 'scene0488_00.txt', 'scene0304_00.txt', 'scene0019_00.txt', 'scene0568_00.txt', 'scene0414_00.txt', 'scene0019_01.txt', 'scene0426_00.txt', 'scene0568_02.txt', 'scene0426_02.txt', 'scene0426_01.txt', 'scene0426_03.txt', 'scene0568_01.txt', 'scene0575_02.txt', 'scene0575_00.txt', 'scene0575_01.txt', 'scene0549_00.txt']
    if opt.scene_id != '':
        pred_files = [filename for filename in pred_files if opt.scene_id in filename]
        pass
    if opt.num_testing_images > 0:
        pred_files = pred_files[:opt.num_testing_images]
        print(pred_files)
        pass
